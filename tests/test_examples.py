...
def test_hello_robot_plays_one_happy_behavior_to_completion() -> None:
    source = (ROOT / "examples" / "hello_robot" / "app.py").read_text(
        encoding="utf-8"
    )

    assert "app.robot.behavior.play," in source
    assert '"happy",' in source
    assert "repeat=1" in source
    assert "job.wait," in source
    assert "20.0" in source
    assert "random" not in source
    assert "while True:" not in source
    assert "app.robot.animation.play" not in source
