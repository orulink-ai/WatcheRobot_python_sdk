...
def test_schema_three_catalog_uses_daemon_protocol_not_application_sdk(provider: str) -> None:
    ...
    hub = FakeMarketplaceHub(
        catalog=_catalog([{"repo_id": FIRST_SPACE, "commit": FIRST_COMMIT}]),
        manifests={(FIRST_SPACE, FIRST_COMMIT): json.dumps(payload).encode()},
    )
    result = load_official_marketplace(
        provider=provider, hub=hub, watcherobot_version="0.1.9"
    )
