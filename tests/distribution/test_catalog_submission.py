...
def _pull_request(repo_id: str, commit: str, number: int = 7):
    return CatalogPullRequest(
        number=number,
        title=catalog_pull_request_title(repo_id, commit),
        url=(
...
def test_first_submission_appends_minimal_entry() -> None:
    ...
    assert json.loads(plan.content or b"") == [
        {"repo_id": SPACE_ID, "commit": COMMIT_A}
    ]
...
def test_new_version_updates_commit_in_place_without_reordering() -> None:
    other = {
        "repo_id": "someone/WatcherRobot-other",
        "commit": "d" * 40,
    }
    plan = plan_catalog_submission(
        _document(
    ...
    assert json.loads(plan.content or b"") == [
        {"repo_id": SPACE_ID, "commit": COMMIT_B},
        other,
    ]
...
def test_same_commit_already_on_main_is_idempotent() -> None:
    plan = plan_catalog_submission(
        _document([{"repo_id": SPACE_ID, "commit": COMMIT_A}]),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["invalid"],
        [{"repo_id": SPACE_ID}],
        [{"repo_id": SPACE_ID, "commit": "main"}],
        [{"repo_id": "not-a-space", "commit": COMMIT_A}],
        [
            {
                "repo_id": SPACE_ID,
                "commit": COMMIT_A,
