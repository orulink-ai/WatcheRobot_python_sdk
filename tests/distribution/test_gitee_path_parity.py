"""Publication and export enforce the same portable path contract."""
...
@pytest.mark.parametrize(
    "paths",
    [
        ("CON.txt",), ("assets/AUX/icon.png",), ("lpt9.log",),
        ("assets./icon.png",), ("assets /icon.png",), ("icon.png ",),
        ("Icon.png", "icon.png"), ("icon.png", "icon.png"),
        (".GIT/config",), ("Assets/a.txt", "assets/b.txt"),
        ("assets/Sub/a.txt", "assets/sub/b.txt"),
        ("assets", "assets/a.txt"), ("assets/a.txt", "assets"),
    ],
)
...
def test_unsafe_paths_rejected_before_io(tmp_path, paths, operation):
    class NoApi:
        def request(self, *args, **kwargs):
            pytest.fail("invalid paths must be rejected before API access")
    ...
    with pytest.raises(HubInvalidResponse):
        if operation == "publish":
            ...
            tree = {"tree": [
                {"path": path, "size": 1, "type": "blob", "mode": "100644", "sha": "a" * 40}
                for path in paths
            ]}
            repository._export_snapshot(
                "alice/app", "a" * 40, tmp_path, tree,
                lambda **kwargs: pytest.fail("invalid paths must fail before reads"),
            )
...
def test_shared_directories_publish_and_export(tmp_path, directories_first):
    ...
    files = [{"path": path, "size": 1, "type": "blob", "mode": "100644", "sha": digest} for path in paths]
    directories = [{"path": path, "type": "tree", "mode": "040000"} for path in ("assets", "assets/sub")]
