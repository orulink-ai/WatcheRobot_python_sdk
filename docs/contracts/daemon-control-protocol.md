...
Daemon 只有在管理接口和业务端口均启动成功后才发布状态文件；退出时先关闭管理接口和业务端口，再仅删除内容仍与本进程所发布状态完全一致的文件。CLI 停止流程会同时等待状态撤销和管理端口关闭，不复用启动冲突判定，也不因阶段性超时将正常关闭误报为失败。端口冲突或部分启动失败不得预先删除、覆盖或在清理阶段移除其他在线实例的状态。
