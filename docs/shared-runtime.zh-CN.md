...
版本更新窗口只显示桌面端版本和包内 Runtime 版本。后者读取 runtime-build.json 的 sdk_version，缺少清单显示不可用；源码模式也不使用当前进程版本冒充内置版本。

## 本轮交付边界

没有增加安装器自动停止旧版进程的钩子，也未完成 Windows 真包 OTA、macOS 实机验收。首次从不支持共享管理协议的旧包迁移，需正常停止旧 Daemon 和应用后覆盖安装，无需删除应用数据。七天回收仅针对可证明无引用的受管目录，当前、回退及应用引用目录继续保留。进程卡死时保守报错，不根据未经核验的 PID 强杀其他进程。掉电事务恢复、旧协议强制迁移和安装器跨进程维护锁不在本轮实现内。
