...
SDK 的 ensure/activate 成功返回后，以及应用安装/卸载完成后，尝试回收共享 bundles 和已登记应用仓库 runtimes 下的内容哈希目录。清理与启动、安装共用生命周期锁和引用锁；自动回收对两把锁都只做非阻塞获取，任一锁繁忙时立即延期，不能拖延已完成的前台操作。无桌面端也执行同一逻辑，不另设清理 Daemon。
