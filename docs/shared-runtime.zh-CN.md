...
Windows OTA 的下载失败不停止现有 Runtime；`prepare-update` 为每次更新返回专属 token，只有持有同一 token 的 `cancel-update` 才能解除排空，避免一个桌面取消另一个桌面的更新。安装失败恢复桌面退出状态，后续由用户重试。操作系统强制终止桌面不等同于正常退出，不能保证执行停止回调。
...
显式启动前仍严格验证现有服务身份。已有健康实例可直接复用，不依赖本地启动器记录；冷启动会使用保存的白名单环境预检最终选中的启动器。候选启动后，TCP 端口监听不代表管理接口已就绪；短暂状态查询失败只继续等待本次候选，不立即判定为未知进程并回滚。仅当返回的 `launch_id` 和候选 `build_id` 均匹配时，才提交启动器记录。候选就绪等待不设固定总时限；启动取消或失败时只清理本次候选进程树，不按端口杀进程。旧 Runtime 回退另有 60 秒上限，超时后回收本次恢复进程树。
