...
候选 Runtime 的就绪等待不设固定总时限，可由启动方通过 WATCHER_RUNTIME_CANCEL_FILE 取消。激活失败后恢复旧 Runtime 最多等待 60 秒。两条路径遇到进程退出、身份不一致等真实错误都会终止；状态接口暂时不可用则继续探测。回退超时或失败时清理本次恢复进程树并释放操作锁。

桌面退出通过本次激活专用的 WATCHER_RUNTIME_CANCEL_FILE 标记取消等待；SDK 只清理自己创建的候选进程树，不提交启动记录、不启动回退实例。资源发布和轻量身份查询按需加载服务依赖；显式激活在停止旧 Runtime 前执行无监听、无状态写入的服务依赖预检。业务路由和版本复用规则不变。
...
显式切换先通过 `--check-runtime` 导入 Daemon 服务依赖并验证候选身份，再阻止新应用启动、停止当前应用和旧 Daemon，等待 Daemon 及冻结启动器释放文件，启动候选并验证 launch_id/build_id，最后原子更新启动记录。候选启动失败时保留旧记录并尝试恢复旧 Runtime；恢复失败报告错误并保留日志。当前实现不提供掉电事务恢复。

## 自动回收与 SDK 独立使用

SDK 的 ensure/activate 成功返回后，以及应用安装/卸载完成后，尝试回收共享 bundles 和已登记应用仓库 runtimes 下的内容哈希目录。清理与启动、安装共用生命周期锁、引用锁和仓库发布锁；自动回收对所有协调锁都只做非阻塞获取，任一锁繁忙时立即延期，不能拖延已完成的前台操作。无桌面端也执行同一逻辑，不另设清理 Daemon。

- 保留 current-launcher.json、previous-launcher.json 所指的版本；后者只在成功切换到不同启动命令时轮换。
- 保留登记的应用目录、虚拟环境 pyvenv.cfg、应用安装记录（包括 staging/trash）引用的版本，以及当前用户进程的可执行文件、打开文件和映射文件引用的版本。启动 Application 前，应用目录与外部虚拟环境必须在同一个引用锁内完成登记和最终校验，不能在两次独立登记之间留下回收窗口。
- 只回收目录修改时间距今超过 7 天、无上述引用的内容哈希目录。链接和 Windows junction 不作为删除目标。
- 进程权限不足、记录损坏、引用目录无法完整枚举、目录不可读或文件占用时跳过，原因记录在实例目录 cleanup-report.json；后续成功操作重试。不存在的引用目录表示尚未登记，已存在但类型错误、不可读或枚举失败则延期整轮回收。无法完整查看进程的机器可能一直延后回收。
...
| SDK 已先启动，再首次安装桌面 | 普通后台连接只复用并观察；安装后的目标 `build_id` 不同时显式执行 `daemon activate` |
| 仅 SDK 创建/下载/安装/运行应用 | 创建下载无需 Daemon；安装登记引用；运行通过共享 Daemon；不依赖桌面 |
| 新架构桌面 OTA | 下载完成后确认应用及 Daemon 已停止，再调用安装器；停止失败取消安装；安装后的目标 `build_id` 不同时显式激活 |
| 手动下载新版覆盖安装 | 共享目录内进程不占安装资源；普通启动只按 SDK 版本策略复用，目标 `build_id` 不同时由 Desktop 显式激活 |
...
锁当前只支持无 extras、无 URL、无条件标记的精确版本，不支持包哈希；包含生成环境内的所有发行包。生成器在替换旧锁前会确认 Manifest 声明的依赖均被兼容 pin 覆盖。跨平台依赖不同的应用应分别验证、限制支持平台，不能把单个平台的锁当作跨平台保证。
...
flowchart TD
    ...
    C -->|无| D[启动候选并验证实例身份]
    C -->|有| E{请求类型?}
    E -->|普通 SDK 启动| F[按 SDK 版本策略复用或切换]
    E -->|Desktop 安装后激活| R{目标 build_id 相同?}
    R -->|是| F
    R -->|否| G[停止应用和旧 Daemon 并等待退出]
    G --> D
    D -->|成功| H[保存当前启动器及回退记录]
    D -->|失败| I[清理候选并尝试恢复旧启动器]
    J[SDK 显式 daemon activate] --> G
    K[桌面后台观察] --> L[重连并提示 不重新启动 Runtime]
    M[桌面正常退出] --> S{仍有其他登记客户端?}
    S -->|是| T[仅释放本客户端登记]
    S -->|否| N[请求 SDK 管理器停止当前共享 Daemon]
    U[Windows OTA 安装前] --> N
    N --> O{确认停止?}
    O -->|是| P[退出或开始安装]
    O -->|否| Q[报错并保留桌面窗口]
...
Windows OTA 的下载失败不停止现有 Runtime；`prepare-update` 为每次更新返回专属 token，只有持有同一 token 的 `cancel-update` 才能解除排空，避免一个桌面取消另一个桌面的更新。Daemon 进入关机流程后 token 立即失效，取消更新不能重新开放 Application 启动入口。安装失败恢复桌面退出状态，后续由用户重试。操作系统强制终止桌面不等同于正常退出，不能保证执行停止回调。
...
显式启动前仍严格验证现有服务身份。已有健康实例可直接复用，不依赖本地启动器记录；冷启动会使用保存的白名单环境预检最终选中的启动器。现有服务发现失败时，候选预检完成后必须先走身份核验的兼容停止路径；无法确认端口属于可安全停止的旧 Runtime 时拒绝启动候选。候选启动后，TCP 端口监听不代表管理接口已就绪；短暂状态查询失败只继续等待本次候选，不立即判定为未知进程并回滚。仅当返回的 `launch_id` 和候选 `build_id` 均匹配时，才提交启动器记录。候选就绪等待不设固定总时限；启动取消或失败时只清理本次候选进程树，不按端口杀进程。旧 Runtime 回退另有 60 秒上限，超时后回收本次恢复进程树。源码运行时的 `build_id` 覆盖包内源码与运行资源，并忽略字节码和缓存目录。

安装维护守卫在取得每一层锁后都会重新检查安装器进程和取消标记。安装器等待超时后会先发布取消标记，并有限等待守卫结束，避免守卫稍后取得锁后再停止已经恢复使用的 Runtime。
