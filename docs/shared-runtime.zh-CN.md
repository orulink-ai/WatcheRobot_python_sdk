...
显式启动前仍严格验证现有服务身份。候选启动后，TCP 端口监听不代表管理接口已就绪；短暂状态查询失败只继续等待本次候选，不立即判定为未知进程并回滚。仅当返回的 `launch_id` 和候选 `build_id` 均匹配时，才提交启动器记录。候选就绪等待不设固定总时限；启动取消或失败时只清理本次候选进程树，不按端口杀进程。旧 Runtime 回退另有 60 秒上限，超时后回收本次恢复进程树。
