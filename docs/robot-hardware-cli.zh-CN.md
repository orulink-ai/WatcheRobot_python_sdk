...
预览和录制采用不同策略。预览允许覆盖旧帧以维持低延迟；录制先按指定 FPS 从更快的摄像头原始流主动采样，已经选中的帧不允许静默丢弃。WebSocket/TCP 会在连接存续期间重传网络丢包，WSPK 分片序号和 WREC 媒体序号负责检测遗漏。CLI 的收帧写盘循环与状态查询、心跳续租分别运行，状态查询卡顿不会占住收帧线程；主机最多暂存 1024 个录制分片（约 16 MiB）。队列溢出、序号缺口、发送失败或断线都会让录制明确失败，并保留 `.wrec.part` 供诊断，不会输出一个伪成功文件。首版不承诺 Wi-Fi 完全断开后的跨连接续录。
