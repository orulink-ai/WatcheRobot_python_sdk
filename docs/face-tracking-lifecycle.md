...
`start()` 默认等待 10 秒：整合固件首次启动需要校验四个模型槽位，真机约 6 秒，超过 Application
传输默认的 5 秒命令期限。可以显式指定 `timeout`；传入 `None` 则使用传输配置的期限。
此调整只影响跟随启动的等待上限，不延长普通命令或退出清理期限。
...
- 启动与清理由同一控制锁串行化；连接进入关闭状态后拒绝新启动。退出最多等待控制锁 2 秒，取得锁后停止请求另有 2 秒上限；取得锁超时会记录警告并继续关闭连接，从而允许传输关闭打断正在等待的启动。此时设备状态不确定，不保证跟随已停止；其他资源各自的关闭耗时不计入这两个上限。
