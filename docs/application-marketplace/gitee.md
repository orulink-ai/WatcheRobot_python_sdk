...
Gitee 发布按 SDK 收集的源文件原始字节构建 Git blob 和索引，不运行 `.gitattributes` 的换行、编码、ident 或过滤器转换；属性文件本身仍作为普通源码保留。提交前核对所有 blob 摘要、文件清单及普通文件模式，不一致则停止，不推送。
...
对齐指 SDK 的应用代码分发流程，不指平台所有能力完全相同：Gitee 需要 Git，不提供 HF Space 同等托管运行能力；本实现未承诺 Git LFS 对象下载。Gitee 的 Windows 安全路径约束也不等价于接受任意 Unix 文件名。
