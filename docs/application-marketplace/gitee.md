...
投稿源码读取和下载会先匿名查询 commit 接口，核对返回的完整 SHA；目录树 SHA、缺失提交或不一致的响应都会在读取文件树前被拒绝。请使用 `publish` 返回的提交 SHA。单个适配器只缓存最近一次验证成功的仓库与 commit 组合，避免清单和图标重复查询；失败结果不缓存。
...
### 2026-09-23 固定版本 commit 身份校验

- 新增 15 项回归测试覆盖文件读取、下载入口、异常响应、限流、缓存隔离及投稿提前停止；分发模块共 386 项测试通过，分发模块 mypy 通过。
- 真实 Gitee 匿名只读复验：前测应用提交 `38df32b33e9986635a0f5b97095de232a4d38b69` 可读取 374 字节清单，其目录树 `50c916d81bf1e14666d3b127bc10ab166d241410` 被拒绝为无效提交。本轮未新增远端写入。
