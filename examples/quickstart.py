"""Interactive quickstart that directly calls the main SDK capabilities."""
...
def main() -> None:
    ...
    with WatcheRobot.connect(pairing_code=pairing_code.strip(), timeout=30.0) as robot:
        print("\nDevice / 设备信息：", robot.device_info)
        print("Capabilities / 设备能力：", ", ".join(robot.capabilities))
        ...
        missing = sorted(
            ...
            if not robot.supports(capability)
        )
        if missing:
            raise RuntimeError(
                "Quickstart cannot run; missing capabilities / 无法运行，缺少能力："
                + ", ".join(missing)
            )

        print("\n[1/7] Play multi-track Behavior / 播放多轨 Behavior")
        robot.behavior.play("happy", repeat=1).wait(timeout=20.0)

        print("\n[2/7] Play animation / 直接播放动画")
        ...
        input(
            "\n[4/7] Robot will move. Clear obstacles and press Enter. / "
            "即将移动机器人；确认周围没有障碍物后按回车继续。"
        )
        ...
        print("\n[5/7] Transfer and play host WAV / 把电脑 WAV 传给机器人播放")
        robot.audio.play_file(SAMPLE_AUDIO).wait(timeout=30.0)

        input(
            "\n[6/7] Camera will capture. Press Enter after consent. / "
            "即将拍照；确认现场人员知情后按回车继续。"
        )
        image = robot.camera.capture(timeout=10.0)
        image_path = image.save(OUTPUT_DIR / "camera.jpg")
        print(f"Photo saved / 照片已保存：{image_path.resolve()}")

        input(
            "\n[7/7] Microphone will record five seconds. Press Enter after consent. / "
            "即将录音五秒；确认现场人员知情后按回车继续。"
        )
        recording = robot.microphone.record(duration=5.0, timeout=8.0)
        recording_path = recording.save(OUTPUT_DIR / "microphone.wav")
        print(
            f"Recording saved / 录音已保存：{recording_path.resolve()}, "
            f"duration / 时长={recording.duration_seconds:.3f}s, "
            f"dropped_frames={recording.dropped_frames}"
        )

        robot.lights.off()

    print("\nQuickstart completed / 全部执行完成。")
