...
const ENGLISH_PHRASES = new Map([
  ...
  ["并发压力测试", "Concurrent Stress Test"],
  ["同时运行动态自定义 UI、连续拍照和音频播放，结束后验证资源释放。", "Run a changing custom UI, repeated camera capture, and audio playback at the same time, then verify resource release."],
  ["开始并发压力测试", "Start Concurrent Stress Test"],
  ["等待测试", "Awaiting Test"],
  ["动态自定义 UI", "Dynamic Custom UI"],
  ["连续拍照", "Repeated Camera Capture"],
  ["音频播放", "Audio Playback"],
  ["并发重叠", "Concurrent Overlap"],
  ["资源恢复", "Resource Recovery"],
  ["等待", "Waiting"],
  ["自定义 UI 持续变化且没有异常闪烁", "Custom UI kept changing without abnormal flicker"],
  ["已从机器人扬声器听到音频", "Audio was audible from the robot speaker"],
  ["正在并发运行：动态自定义 UI + 连续拍照 + 音频播放…", "Running concurrently: dynamic custom UI + repeated photos + audio playback…"],
  ["SDK 自动阶段全部通过，请人工确认屏幕与扬声器输出。", "All SDK stages passed. Confirm the screen and speaker output manually."],
  ["综合测试失败于", "Combined test failed at"],
  ["未知错误", "unknown error"],
  ["未知", "unknown"],
  ["通过", "Passed"],
  ["跳过", "Skipped"],
  ...
  ["开机以来最低内部 RAM", "Boot Minimum Internal RAM"],
  ["最近功能资源快照", "Recent Feature Resource Snapshots"],
  ["任务栈大小表示仍驻留的分配，并非该功能的全部内存。完整切换记录会打印到设备日志。", "Worker stack sizes describe resident allocations, not total memory owned by a feature. Full transition history is printed to the device log."],
  ["阶段", "Stage"],
  ["TTS 任务栈", "TTS Stack"],
  ["音效任务栈", "SFX Stack"],
  ["占用状态", "Owners"],
